...
function onGameLoad() {
  ...
  north_arrow.addEventListener("click", function() { if (this.classList.contains("directionAvailable")) { move([0,1,0]); allowedLines.push(room + ",vertical"); drawLines(); removeArrowColorings(this,""); }});
  south_arrow.addEventListener("click", function() { if (this.classList.contains("directionAvailable")) { allowedLines.push(room + ",vertical"); move([0,-1,0]); drawLines(); removeArrowColorings(this,""); }});
  east_arrow.addEventListener("click", function() { if (this.classList.contains("directionAvailable")) { allowedLines.push(room + ",horizontal"); move([1,0,0]); drawLines(); removeArrowColorings(this,""); }});
  west_arrow.addEventListener("click", function() { if (this.classList.contains("directionAvailable")) { move([-1,0,0]); allowedLines.push(room + ",horizontal"); drawLines(); removeArrowColorings(this,""); }});
  northwest_arrow.parentNode.addEventListener("click", function() { if (this.children[0].classList.contains("directionAvailable")) { allowedLines.push(room + ",downhill"); move([-1,1,0]); drawLines(); removeArrowColorings(this.children[0],""); }});
  northeast_arrow.parentNode.addEventListener("click", function() { if (this.children[0].classList.contains("directionAvailable")) { allowedLines.push(room + ",uphill"); move([1,1,0]); drawLines(); removeArrowColorings(this.children[0],""); }});
  southwest_arrow.parentNode.addEventListener("click", function() { if (this.children[0].classList.contains("directionAvailable")) { move([-1,-1,0]); allowedLines.push(room + ",uphill"); drawLines(); removeArrowColorings(this.children[0],""); }});
  southeast_arrow.parentNode.addEventListener("click", function() { if (this.children[0].classList.contains("directionAvailable")) { move([1,-1,0]); allowedLines.push(room + ",downhill"); drawLines(); removeArrowColorings(this.children[0],""); }});
...
function removeArrowColorings(item,delStatus) {
  ...
  surroundings_btn.innerHTML = 'SURROUNDINGS <b>(0)</b>';
...
function addRoomObject(string) {
  ...
  if (cntr === 0) {
    ...
    var currentCount = parseInt(surroundings_btn.innerHTML.replace('SURROUNDINGS <b>(','').replace(')</b>',''));
    currentCount++;
    surroundings_btn.innerHTML = 'SURROUNDINGS <b>(' + currentCount + ')</b>';
  }
}
    ...
    for (var x = 0; x < contentBoxes.length; x++) {
      ...
      for (var y = 0; y < panels.length; y++) {
        ...
        panels[y].onclick = function() {
          dom_scene.innerHTML += "<span class='command'> &gt SURROUNDINGS -> " + this.roomObj.name.toUpperCase() + " -> " + this.actionsName + "</span><br />";
          dom_scene.scrollTo(0, dom_scene.scrollHeight);
          this.action();
          dom_sidePanel.style.right = "-70%";
          document.getElementById("surroundings_btn").classList.remove("buttonActive"); };
...
function updateStats() {
  ...
  dom_mask.innerHTML = "<div class='hedda'>STATISTICS</div>";
  var ul = document.createElement("UL");
  dom_mask.appendChild(ul);
  for (var i = 0; i < statistics.length; i++) {
    dom_mask.children[1].innerHTML += "<li>" + statistics[i].name.replace(/_/g, " ").toUpperCase() + ": " + eval("pl." + statistics[i].name) + statistics[i].symbol + "</li>";
  }
}
function showSettings() {
  dom_mask.style.textAlign = "center";
  dom_mask.innerHTML = "<div class='hedda'>SETTINGS</div><br>";
  if (ornt === "portrait") dom_mask.innerHTML += "<div><u>Map Opacity</u></div><br><div class='slideContainer'><input type='range' step='0.1' min='0' max='1' value='" + opacity + "' id='opacitySlider' class='slider'></div><div>(This option allows you to see the map through the console window: 0 = map not visible, 1 = map fully visible.)</div><br><br>";
  dom_mask.innerHTML += "<div><u>Font Size</u></div><br><div class='slideContainer'><input type='range' step='0.1' min='0.8' max='1.8' value='" + fontSize + "' id='fontSizeSlider' class='slider'></div><div>(This option lets you change the font size of text in the console.)</div><br><br>";
  dom_mask.innerHTML += "<div><u>Save/Load Game</u></div><br>Choose a slot: &nbsp;<select></select><br><br>";
  ...
  function updateDropdown() {
    ...
    for (var i = 0; i < slotsLen; i++) {
      dropdown.innerHTML += "<option>" + saveSlots[i] + "</option>";
    }
  }

  updateDropdown();

  dom_mask.innerHTML += "<div class='button' id='saveButton' style='margin: 0 auto; width: 9em;'>SAVE GAME</div><div>(This will save the game as it was <i>before</i> you entered the room you're currently in.)</div><br><br>";
  dom_mask.innerHTML += "<div class='button' id='loadButton' style='margin: 0 auto; width: 9em;'>LOAD GAME</div><div>(This will load the game.)</div><br><br>";
  dom_mask.innerHTML += "<div><u>Clear Console</u></div><br>";
  dom_mask.innerHTML += "<div class='button' id='clearButton' style='margin: 0 auto; width: 13em;'>CLEAR CONSOLE</div><div>(Game getting sluggish? This button will clear the console and speed it up again!)</div><br><br>";
...
function saveGame(command) {
  if (sessionStorage.getItem("room") !== null) {
    if (command === "Create a new save slot...") {
      var name = prompt("Please choose a name for the save file. Only letters are allowed. No spaces.");
...
function loadGame(command) {
  if (command === "Create a new save slot...") {
    ...
    if (localStorage.getItem("room" + "$" + command) !== null) {
